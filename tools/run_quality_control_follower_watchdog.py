#!/usr/bin/env python3
"""Recover the CPU-only quality follower without duplicating a live job."""
...
FOLLOWER_SCRIPT = (PROJECT_ROOT / "tools" / "run_quality_control_follower.py").resolve()
...
def quality_complete(state: dict[str, object]) -> bool:
    readiness = state.get("freeze_readiness")
    if state.get("status") != "COMPLETE" or not isinstance(readiness, dict):
        return False
    try:
        completed = int(state.get("completed_sequence_count", -1))
        total = int(state.get("total_sequence_count", -2))
        ready = int(readiness.get("ready_sequence_count", -3))
        ready_total = int(readiness.get("total_sequence_count", -4))
    ...
    return bool(
        ...
        and completed == total
        and ready == total
        and ready_total == total
        and not state.get("failures")
        and not readiness.get("failures")
    )


def validate_recovery_command(argv: list[str] | None) -> str | None:
    if argv is None:
        return "quality follower resume command is missing"
    if "--once" in argv:
        return "quality follower resume command must not contain --once"
    if "--sequences" not in argv:
        return "quality follower resume command has no --sequences"
    return None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--lock-path",
        type=Path,
        default=PROJECT_ROOT / ".runtime" / "quality_follower_watchdog.lock",
    )
    parser.add_argument(
        "--log-path",
        type=Path,
        default=PROJECT_ROOT / ".runtime" / "quality_follower_recovery.log",
    )
...
def main() -> int:
    ...
    if singleton is None:
        print("quality follower watchdog is already running", flush=True)
        return 3
    ...
    while True:
        ...
        complete = quality_complete(follower_state)
        ...
        if argv is None and command_error is not None:
            command_error = (
                "persisted quality follower resume command is invalid: "
                + command_error
            )
        validation_error = validate_recovery_command(argv)
        ...
        if len(processes) == 1 and not live_identity_ok:
            ...
            command_error = "live quality follower does not match pinned identity"

        if decision == "ATTENTION_DUPLICATE":
            attention_reasons.append(
                {
                    "code": "DUPLICATE_QUALITY_FOLLOWER",
                    "message": (
                        f"{len(processes)} quality followers are alive; "
            ...
            attention_reasons.append(
                {
                    "code": "QUALITY_FOLLOWER_RECOVERY_IDENTITY_INVALID",
            ...
            attention_reasons.append(
                {
                    "code": "QUALITY_FOLLOWER_RESTART_EXHAUSTED",
                    "message": (
            ...
            last_event = "CONFIRMING_QUALITY_FOLLOWER_ABSENCE"
        elif decision == "COMPLETE":
            missing_observations = 0
            last_event = "QUALITY_COMPLETE_NO_RESTART"
            ...
            if final_processes:
                ...
                try:
                    ...
                    missing_observations = 0
                    last_event = "QUALITY_FOLLOWER_RESTART_LAUNCHED"
                except OSError as error:
                    attention_reasons.append(
                        {
                            "code": "QUALITY_FOLLOWER_RESTART_FAILED",
                            "message": (
                                f"follower launch failed: {type(error).__name__}: {error}"
                            ),
                        }
                    )
                    last_event = "QUALITY_FOLLOWER_RESTART_FAILED"
        ...
        state = {
            ...
            "max_restarts": args.max_restarts,
            "quality_follower_status": follower_state.get("status", "UNKNOWN"),
            ...
            "policy": (
                ...
                "absence; final rescan; singleton watchdog and follower lifetime locks; capped "
                "detached recovery; stop after validated all-sequence freeze readiness"
            ),
        }
