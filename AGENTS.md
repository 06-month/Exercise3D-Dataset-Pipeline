...
2. ignored local state `.runtime/handoff_state.json`, `.runtime/dashboard_state.json`,
   `.runtime/supervisor_watchdog_state.json`,
...
8. supervisor가 죽었다면 먼저 supervisor watchdog이 살아 있는지 확인한다. Watchdog은
   exact command identity, 3회 연속 absence, final rescan, singleton lock을 통과한 때만 자동
   resume한다. Watchdog이 정상이면 agent가 동일 command를 수동 실행하지 않는다.
